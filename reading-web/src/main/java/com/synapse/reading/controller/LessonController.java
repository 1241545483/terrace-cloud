...
public class LessonController extends BaseController {
	...
	@ApiImplicitParams({ @ApiImplicitParam(name = "recId", paramType = "query"),
	...
	public ResponseEntity list(PageInfo pageInfo, @Validated(Search.class) LessonParam param,
	...
	@ApiResponses({ @ApiResponse(code = 200, response = String.class, message = "主键"),
			@ApiResponse(code = 1002, response = String.class, message = "字段校验错误"),
			@ApiResponse(code = 500, response = String.class, message = "服务器错误") })
	...
	@ApiResponses({ @ApiResponse(code = 200, response = Integer.class, message = "删除数量"),
			@ApiResponse(code = 500, response = String.class, message = "服务器错误") })
	...
	@ApiResponses({ @ApiResponse(code = 200, response = Integer.class, message = "更新数量"),
			@ApiResponse(code = 1002, response = String.class, message = "字段校验错误"),
			@ApiResponse(code = 500, response = String.class, message = "服务器错误") })
	@RequestMapping(value = "/v1/lesson/{recId}", method = RequestMethod.PUT)
	public ResponseEntity update(@PathVariable("recId") String recId,
			@RequestBody @Validated(Update.class) LessonParam param, BindingResult bindingResult) {
		try {
			// 验证失败
			if (bindingResult.hasErrors()) {
				throw new ValidException(bindingResult.getFieldError().getDefaultMessage());
	...
	public ResponseEntity updatByorderNum(@RequestBody @Validated(Update.class) List<OrderNumParam> list,
			BindingResult bindingResult) {
		try {
			// 验证失败
			if (bindingResult.hasErrors()) {
	...
	public ResponseEntity createLesson(@RequestBody @Validated(Create.class) LessonParam param,
			BindingResult bindingResult) {
		try {
			// 验证失败
			if (bindingResult.hasErrors()) {
	...
	public ResponseEntity updateLesson(@RequestBody @Validated(Create.class) LessonParam param,
			BindingResult bindingResult) {
		try {
			// 验证失败
			if (bindingResult.hasErrors()) {
	...
	@ApiResponses({ @ApiResponse(code = 200, response = LessonResult.class, message = "是否查询到记录"),
			@ApiResponse(code = 500, response = String.class, message = "服务器错误") })
	...
	@ApiOperation(value = "根据主键查询已收藏的lesson详情")
	@ApiResponses({ @ApiResponse(code = 200, response = LessonResult.class, message = "lesson对象"),
			@ApiResponse(code = 1002, response = String.class, message = "字段校验错误"),
			@ApiResponse(code = 500, response = String.class, message = "服务器错误") })
	@RequestMapping(value = "/v1/lesson/selectIsCollect/{recId}", method = RequestMethod.GET)
	...
	@ApiResponses({ @ApiResponse(code = 200, response = LessonResult.class, message = "收藏的课程列表"),
			@ApiResponse(code = 1002, response = String.class, message = "字段校验错误"),
			@ApiResponse(code = 500, response = String.class, message = "服务器错误") })
	...
	public ResponseEntity listMyCollectByLesson() {
		try {
			User user = UserContext.getUser();
			// todo 根据角色判断权限
			List<LessonResult> results = lessonService.listMyCollectByLesson(user);
			return ResponseEntity.ok(results);
		} catch (BusinessException e) {
			logger.error("get lesson Error!", e);
			return ResponseEntity.status(CommonConstants.SERVER_ERROR).body(Result.error(e));
